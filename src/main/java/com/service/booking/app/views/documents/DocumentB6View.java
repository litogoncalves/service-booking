...
@PageTitle("Documento B6 | SIGAV - Sistema de Gestão de Agendamentos e Validações")
@Route(value = "documentB6", layout = MainLayout.class)
@AnonymousAllowed
public class DocumentB6View extends VerticalLayout {
	...
	public DocumentB6View(ServService servService, BookingService bookingService, IdentityDocumentService identityDocService, 
	...
	private Component createHeader() {
		headerDiv = new Div();
		header = new H4(Labels.DOCUMENT_TYPE_TRIP_B6);
	...
	private Service createService() {
		Service service = servService.getServiceByName(Constants.DOCUMENT_B6);
		return service;
	}
	...
	private Component createButtons() {
	 ...
	 private void createConfirmDialog() {
		 try {
			 ...
			 if(binder.isValid()) {
				 ...
				 confirmDialog.add(new H5(Labels.BOOKING_CONFIRMATION+" "+Labels.DOCUMENT_TYPE_TRIP_B6));
	 ...
	 private void notifyAfterBooking() {
		 ...
		 notificationManager.sendSMSBookingNotification(booking, Notifications.SMS_BOOKING_SUCCESS.replace("#name", firstNameReq.concat(" "+booking.getSurnameReq()))
				 .replace("#code", booking.getBookingId()).replace("#doc", Labels.EXIT_DECLARATION).replace("#local", booking.getLocation().getName())
