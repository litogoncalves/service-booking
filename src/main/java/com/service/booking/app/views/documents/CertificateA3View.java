package com.service.booking.app.views.documents;
...
@PageTitle("Certificado A3 | SIGAV - Sistema de Gestão de Agendamentos e Validações")
@Route(value = "certificateA2", layout = MainLayout.class)
@AnonymousAllowed
public class CertificateA3View extends VerticalLayout{
	...
	public CertificateA3View(ServService servService, BookingService bookingService, IdentityDocumentService identityDocService, 
	...
	private Component createHeader() {
		headerDiv = new Div();
		header = new H4(Labels.DOCUMENT_TYPE_EMERGENCY_CERTIFICATE_A3);
	...
	private Service createService() {
		Service service = servService.getServiceByName(Constants.DOCUMENT_A3);
		return service;
	}
	...
	private Component createButtons() {
	 ...
	 private void createConfirmDialog() {
		 try {
			 ...
			 if(binder.isValid()) {
				 ...
				 confirmDialog.add(new H5(Labels.BOOKING_CONFIRMATION+" "+Labels.DOCUMENT_TYPE_EMERGENCY_CERTIFICATE_A3));
	 ...
	 private void notifyAfterBooking() {
		 ...
		 notificationManager.sendSMSBookingNotification(booking, Notifications.SMS_BOOKING_SUCCESS.replace("#name", firstNameReq.concat(" "+booking.getSurnameReq()))
				 .replace("#code", booking.getBookingId()).replace("#doc", Labels.EMERGENCY_CERTIFICATE).replace("#local", booking.getLocation().getName())
