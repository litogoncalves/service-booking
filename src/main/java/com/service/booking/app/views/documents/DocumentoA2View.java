...
@PageTitle("Documento A2 | SIGAV - Sistema de Gestão de Agendamentos e Validações")
@Route(value = "documentA2", layout = MainLayout.class)
@AnonymousAllowed
public class DocumentoA2View extends VerticalLayout {
	...
	public DocumentoA2View(ServService servService, BookingService bookingService, IdentityDocumentService identityDocService, 
	...
	private Component createHeader() {
		headerDiv = new Div();
		header = new H4(Labels.DOCUMENT_TYPE_TRAVEL_A2);
	...
	private Service createService() {
		Service service = servService.getServiceByName(Constants.DOCUMENT_A2);
		return service;
	}
	...
	private Component createButtons() {
	 ...
	 private void createConfirmDialog() {
		 try {
			 ...
			 if(binder.isValid()) {
				 ...
				 confirmDialog.add(new H5(Labels.BOOKING_CONFIRMATION+" "+Labels.DOCUMENT_TYPE_TRAVEL_A2));
	 ...
	 private void notifyAfterBooking() {
		 ...
		 notificationManager.sendSMSBookingNotification(booking, Notifications.SMS_BOOKING_SUCCESS.replace("#name", firstNameReq.concat(" "+booking.getSurnameReq()))
				 .replace("#code", booking.getBookingId()).replace("#doc", Labels.DOCUMENT_TYPE_TRAVEL).replace("#local", booking.getLocation().getName())
