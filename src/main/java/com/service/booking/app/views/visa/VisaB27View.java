...
@PageTitle("Visa B27 | SIGAV - Sistema de Gestão de Agendamentos e Validações")
@Route(value = "visaB27", layout = MainLayout.class)
@AnonymousAllowed
public class VisaB27View extends VerticalLayout {
	...
	private ComboBox<GeneralData> lengthOfAdditionalDays;
	...
	public VisaB27View(ServService servService, BookingService bookingService, IdentityDocumentService identityDocService, 
	...
	private Component createHeader() {
		headerDiv = new Div();
		header = new H4(Labels.DOCUMENT_TYPE_VISA_B27);
	...
	private Component createFormLayout() {
		...
		formLayout.add(countryOfBirthReq, nationality, hr6, idDocument,idNumber, idIssue, idValidate, passportNationality);
		formLayout.add(hr7, profession ,professionReq ,positionReq ,companyReq);
		formLayout.add(hr2);
		formLayout.add(accommodationAddress);
		formLayout.add(provinceAddress, districtAddress, cityAddress, streetAddress, lengthOfAdditionalDays);
		formLayout.add(hr8, visaTitle, visaReq, visaIssueDate, visaValidate);
		formLayout.add(hr3);
		...
		formLayout.setColspan(lengthOfAdditionalDays, 2);
	...
	private Service createService() {
		Service service = servService.getServiceByName(Constants.VISA_B27);
		return service;
	}
	...
	private void createLengthOfStayMoz() {
		...
		lengthOfAdditionalDays.setItems(lengthOfStayItems);
		lengthOfAdditionalDays.setItemLabelGenerator(GeneralData::getName);
	}
	...
	private void createVariables() {
		...
		this.reasonForTravel = new TextArea(Labels.REASON_FOR_REQUEST_TO_STAY);
        this.reasonForTravel.setMaxLength(rfTravelCharLimit);
        this.reasonForTravel.setValueChangeMode(ValueChangeMode.EAGER);
        this.reasonForTravel.setHelperText("0/"+rfTravelCharLimit+" "+Labels.REASON_FOR_REQUEST_TO_STAY_HELPER_TEXT);
       ...
       this.dateToSchedule = new DatePicker(Labels.DATE_TO_SCHEDULE);
        ...
        this.lengthOfAdditionalDays = new ComboBox<GeneralData>(Labels.ADDITIONAL_PERIOD_OF_STAY);
        this.lengthOfAdditionalDays.setRequired(true);
	 ...
	 private void createConfirmDialog() {
		 try {
			 ...
			 if(binder.isValid()) {
				 ...
				 confirmDialog.add(new H5(Labels.BOOKING_CONFIRMATION+" "+Labels.DOCUMENT_TYPE_VISA_B27));
