...
@PageTitle("Documento B9 | SIGAV - Sistema de Gestão de Agendamentos e Validações")
@Route(value = "documentB9", layout = MainLayout.class)
@AnonymousAllowed
public class DocumentB9View extends VerticalLayout {
	...
	public DocumentB9View(ServService servService, BookingService bookingService, IdentityDocumentService identityDocService, 
	...
	private Component createHeader() {
		headerDiv = new Div();
		header = new H4(Labels.DOCUMENT_TYPE_TRIP_B9);
	...
	private Component createFormLayout() {
		...
		formLayout.add(hr13, otherData, hr14);
	...
	private Service createService() {
		Service service = servService.getServiceByName(Constants.DOCUMENT_B9);
		return service;
	}
	...
	private Component createButtons() {
	 ...
	 private void createConfirmDialog() {
		 try {
			 ...
			 if(binder.isValid()) {
				 ...
				 confirmDialog.add(new H5(Labels.BOOKING_CONFIRMATION+" "+Labels.DOCUMENT_TYPE_TRIP_B9));
	 ...
	 private void notifyAfterBooking() {
		 ...
		 notificationManager.sendSMSBookingNotification(booking, Notifications.SMS_BOOKING_SUCCESS.replace("#name", firstNameReq.concat(" "+booking.getSurnameReq()))
				 .replace("#code", booking.getBookingId()).replace("#doc", Labels.REQUEST_RESIDENCE_DELARATION).replace("#local", booking.getLocation().getName())
