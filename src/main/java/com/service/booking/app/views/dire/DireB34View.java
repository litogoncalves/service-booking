package com.service.booking.app.views.dire;
...
@PageTitle("Dire B34 | SIGAV - Sistema de Gestão de Agendamentos e Validações")
@Route(value = "direB34", layout = MainLayout.class)
@AnonymousAllowed
public class DireB34View extends VerticalLayout {
	...
	public DireB34View(ServService servService, BookingService bookingService, IdentityDocumentService identityDocService, 
	...
	private Component createFormLayout() {
		...
		formLayout.add(hr9, profession ,academicLevel, professionReq, employer, employerAddress, workPhoneNumber, otherActivities);
		formLayout.add(hr2);
	...
	private Service createService() {
		Service service = servService.getServiceByName(Constants.DIRE_B34);
		return service;
	}
...
*/	
		...
		this.fatherNationality.setHelperText(Labels.TYPE_FATHER_NAME_HELPER_TEXT);
		
		this.motherNationality = new ComboBox<Nationality>(Labels.MOTHER_NATIONALITY);
		this.motherNationality.setHelperText(Labels.TYPE_FATHER_NAME_HELPER_TEXT);

		this.spouseNationality = new ComboBox<Nationality>(Labels.SPOUSE_NATIONALITY);
		this.spouseNationality.setHelperText(Labels.TYPE_FATHER_NAME_HELPER_TEXT);
		...
		//this.otherActivities.setHelperText(Labels.TYPE_YOUR_WORK_PHONE_NUMBER);
	...
	private Component createButtons() {
	 ...
	 private void createConfirmDialog() {
		 try {
			 ...
			 if(binder.isValid()) {
				 ...
				 confirmDialog.add(new H5(Labels.BOOKING_CONFIRMATION+" "+Labels.DOCUMENT_TYPE_DIRE_B34));
	 ...
	 private void notifyAfterBooking() {
		 ...
		 notificationManager.sendSMSBookingNotification(booking, Notifications.SMS_BOOKING_SUCCESS.replace("#name", firstNameReq.concat(" "+booking.getSurnameReq()))
				 .replace("#code", booking.getBookingId()).replace("#doc", Labels.DIRE).replace("#local", booking.getLocation().getName())
